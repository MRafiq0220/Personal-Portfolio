...
<%
    ...
    if (username == null || username.isEmpty() || phoneNumber == null || phoneNumber.isEmpty() || address == null || address.isEmpty()) {
        out.println("<script>alert('All fields are required'); window.location.href='updateprofile.jsp';</script>");
        return;
    }
 ...
 conn = DriverManager.getConnection("jdbc:mysql://localhost/sql_mrafiq", "sql_mrafiq", "yDB5npdN8jXaZpL8");
        ...
        if (result > 0) {
            out.println("<script>alert('Profile updated successfully'); window.location.href='updateprofile.jsp';</script>");
        } else {
            out.println("<script>alert('Failed to update profile'); window.location.href='updateprofile.jsp';</script>");
        }
    } catch (Exception e) {
